...
| <img src="docs/flow-dark.png" width="340" alt="Тёмная тема"> | **Тёмная тема.** Та же сетка, те же размеры и радиусы, инвертированная палитра; алый акцент не меняется. Переключается кнопкой в статус-строке, пунктом «Тема» в меню значка или следом за настройкой Windows. |
...
Значок следует теме панели задач Windows, а не теме окна: на светлой панели контур тёмный и поле белое, на тёмной — контур светлый, а поле прозрачное. Алая заливка одинакова в обеих темах.

### Тема

Кнопка в правом углу статус-строки переключает тему на месте: в светлой нарисована луна, в тёмной — солнце, значок показывает, куда переключит. Меню значка в трее даёт третий вариант — «Как в системе»: тема следует за настройкой Windows и меняется на ходу, без перезапуска. Выбор хранится в `settings.json`.
...
| `Theme` | `system` | Тема окна: `light`, `dark` или `system` — следом за Windows |
...
Окно фиксированное, 430×720, без полноэкранного режима и горизонтальной прокрутки. Тем две, светлая и тёмная, с общей сеткой и общим акцентом — третьей не будет. Шрифт — только системный Segoe UI Variable. Всё это не временные упрощения, а ограничения, из которых вырос дизайн: виджет обязан читаться мгновенно и исчезать, как только перестал быть нужен.
